...
# Model Settings
num_node_features = graph_data_objects[
    0
].num_node_features  #  Retrieves the number of features associated with the nodes
...
model = load_or_initialize_model(
    model_path, model_type, num_node_features, num_classes, dropout_rate=0.5
)  # SPECFIY DROPOUT RATE HERE
...
for idx, data in enumerate(graph_data_objects):
    try:  # Start of the try block
        ...
        num_labeled_nodes = num_labeled_nodes_list[
            idx
        ]  # Retrieve the number of labeled nodes for the current graph
        ...
        extended_train_mask = torch.cat(
            (
                train_mask,
                torch.ones(
                    num_total_nodes - num_labeled_nodes, dtype=torch.bool, device=device
                ),
            )
        )
        ...
        for epoch in range(100):  # Adjust the number of epochs as needed
            try:  # Nested try for the inner loop
                train_loss = train(
                    model, optimizer, data, extended_train_mask
                )  # Assuming extended masks are used
                ...
                print(
                    f"Graph {idx+1}, Epoch {epoch+1}: Train Loss: {train_loss:.2f}, Test Loss: {test_loss:.2f}"
                )
            except RuntimeError as e:  # Catch specific runtime errors
                print(
                    f"Encountered an error in Graph {idx+1}, Epoch {epoch+1}: {e}. Skipping to the next epoch."
                )
                continue  # Skip to the next epoch

    except Exception as e:  # Catch other exceptions
        print(
            f"Encountered an error processing Graph {idx+1}: {e}. Skipping to the next graph."
        )
